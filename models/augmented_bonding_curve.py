...
TOTAL_INITIAL_TECH_SUPPLY = 2041.86358
        ...
        for index, step in enumerate(steplist):
            ...
            if token_type == "wxDAI":
                ...
                slippage = (amount_in - tribute_collected) / \
                    bondingCurve.get_price(current_supply) - amount_out
                slippage_pct = slippage / \
                    ((amount_in - tribute_collected) /
                     bondingCurve.get_price(current_supply))
                ...
                # because we are reducing the supply (burning)
                amount_in = amount_in * -1
                amountBeforeTribute = bondingCurve.sale_return(amount_in)

                # since it is a sale, the number returned is negative
                tribute_collected = amountBeforeTribute * bondingCurve.exit_tribute
                ...
                slippage = ((amount_in*(1-bondingCurve.exit_tribute)) *
                            bondingCurve.get_price(current_supply) - amount_out) * -1
                slippage_pct = slippage / \
                    ((amount_in*(1-bondingCurve.exit_tribute)) *
                     bondingCurve.get_price(current_supply)) * -1
                slippage_pct = str(format((slippage_pct*100), '.2f')) + "%"
